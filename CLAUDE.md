...
- **Nothing is charged from the totem yet.** The customer picks efectivo or Mercado Pago at checkout and the order is stored with that choice, but no money moves: whoever attends marks the order as cobrado in the kitchen, and that is what the cash close counts. The Mercado Pago integration (a real checkout, a webhook) is the piece still missing, and it needs the account's credentials.
